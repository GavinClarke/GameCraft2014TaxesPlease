...
PrismaticPlatform::PrismaticPlatform(b2World* world, SDL_Renderer* gRenderer, float leftBound, float rightBound, float yPosition, float speed, b2Vec2 dimensions)
	...
	anchor = ObjectFactory::instance()->createPlatform(world, b2Vec2(leftBound * PIXELSTOMETRES, yPosition * PIXELSTOMETRES), b2Vec2(0.01, 0.01), 0, b2_staticBody, 0.1f);
	anchor->SetUserData((void*)-2);
	b2Filter f; f.groupIndex = -1; anchor->GetFixtureList()[0].SetFilterData(f);

	platform = ObjectFactory::instance()->createPlatform(world, b2Vec2(rightBound * PIXELSTOMETRES, yPosition * PIXELSTOMETRES), dimensions, 0, b2_dynamicBody, 1.0f);
	...
	m_joint->SetMotorSpeed(speed * PIXELSTOMETRES);
...
void PrismaticPlatform::Draw(SDL_Renderer* gRenderer, b2Vec2 offset) { 
	...
	stretchRect.x = (platform->GetPosition().x * METRESTOPIXELS) - (size.x / 2) - offset.x;
	stretchRect.y = -(platform->GetPosition().y * METRESTOPIXELS) - (size.y / 2) + offset.y;
