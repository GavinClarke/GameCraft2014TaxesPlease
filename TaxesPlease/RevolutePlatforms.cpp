...
RevolutePlatforms::RevolutePlatforms(b2World* world, SDL_Renderer* gRenderer, b2Vec2 position, b2Vec2 dimensions, float radius, float speed) {
	size = dimensions;
	m_texture = SDL_CreateTextureFromSurface( gRenderer, IMG_Load( "RevolutePlatform.png") );
	...
	m_centreJoint->SetMotorSpeed(speed*PIXELSTOMETRES);
...
void RevolutePlatforms::Draw(SDL_Renderer* gRenderer, b2Vec2 offset) { 
	...
	stretchRect.x = (platformOne->GetPosition().x * METRESTOPIXELS) - (size.x / 2) - offset.x;
	stretchRect.y = -(platformOne->GetPosition().y * METRESTOPIXELS) - (size.y / 2) + offset.y;
